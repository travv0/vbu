...
let printGame game newName newPath newGlob =
    ...
    | None, Some _ ->
        printConfigRow
            "Save glob"
            (Option.defaultValue "" game.Glob)
            (Some(Option.defaultValue "" newGlob))
    | _ -> ()
            ...
            let globMatches () =
                let glob =
                    Glob.Parse(
                        Path.Join(
                            basePath,
                            Option.defaultValue defaultGlob glob
                        )
                    )
            ...
            let copyAndCleanup () =
                monad {
                    Directory.CreateDirectory(
                        Path.GetDirectoryName(toPath : string)
                    )
                    |> ignore
            ...
            if Directory.Exists game.Path then
                let! backedUpCount, warnings =
                    backupFiles
                        ...
                        game.Glob
                        game.Path
                        (Path.Join(config.Path, gameName))
                        verbose
...
let printConfig newBackupDir newBackupFreq newBackupsToKeep : App<unit> =
    monad {
        ...
        printConfigRow
            ...
            (string <!> newBackupFreq)

        printConfigRow
            "Number of backups to keep"
            (string config.NumToKeep)
            (string <!> newBackupsToKeep)
...
let app (parseResults : ParseResults<_>) : App<Config option> =
    monad {
        ...
        return!
            ...
            | Backup sp ->
                backup
                    (sp.TryGetResult BackupArgs.Games)
                    (sp.Contains Loop)
                    (sp.Contains Verbose)
            | Add sp ->
                add
                    (sp.GetResult AddArgs.Game)
                    (sp.GetResult AddArgs.Path)
                    (sp.TryGetResult AddArgs.Glob)
            | List _ -> list ()
            | Info sp -> info (sp.TryGetResult InfoArgs.Games)
            | Remove sp ->
                remove
                    (NonEmptyList.ofList (sp.GetResult Games))
                    (sp.Contains Yes)
            | Edit sp ->
                edit
                    (sp.GetResult Game)
            ...
            | Config sp ->
                editConfig
                    (sp.TryGetResult Path)
                    (sp.TryGetResult Frequency)
                    (sp.TryGetResult Keep)
            | Config_Path _
            | Version ->
                failwithf $"non-command matched as command: %A{command}"
    }
