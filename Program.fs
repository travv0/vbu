...
let info (groupNames: option<list<string>>) =
    reader {
        ...
        let groups =
            ...
            | None -> config.Groups
            | Some gs ->
                Array.filter (fun g -> List.contains g.Name gs) config.Groups
    ...
    reader {
        ...
        | _ ->
            let splitList =
                ...
                |> Option.map
                    (fun i -> config.Groups |> Array.toList |> List.splitAt i)
                ...
                if not (Group.isValidName newName) then
                    ...
                    return
                        Some
                            { config with
                                  Groups =
                                      front @ editedGroup :: back
                                      |> List.toArray }
    }

let editConfig
    (backupDir: option<string>)
    (backupFreq: option<int>)
    (backupsToKeep: option<int>)
    =
    reader {
        let! config = ask
...
let app (parseResults: ParseResults<_>) =
    ...
    | Backup sp ->
        backup
            (sp.TryGetResult BackupArgs.Groups)
            (sp.Contains Loop)
            (sp.Contains Verbose)
            (sp.Contains Force)
    | Add sp ->
        add
            (sp.GetResult AddArgs.Group)
